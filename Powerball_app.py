...
def load_tickets_from_db():
    ...
    if not df.empty:
        ...
        df = pd.DataFrame(columns=["ID", "Main Numbers", "PowerBall", "Ticket Cost (ZAR)", "Winnings (ZAR)", "Net Profit/Loss"])
    return df
...
def save_bulk_df_to_db(df_to_append):
    ...
    for _, row in df_to_append.iterrows():
        cursor.execute("""
            INSERT INTO tickets (main_numbers, powerball, cost, winnings, net_balance)
            VALUES (?, ?, ?, ?, ?)
        """, (str(row['Main Numbers']), int(row['PowerBall']), float(row['Ticket Cost (ZAR)']), float(row['Winnings (ZAR)']), float(row['Net Profit/Loss'])))
    conn.commit()
    conn.close()
...
def calculate_matches(ticket_numbers_str, ticket_pb, official_main_list, official_pb):
    try:
        ticket_nums = set([int(x.strip()) for x in str(ticket_numbers_str).split(",")])
    except:
...
def send_budget_email(to_email, df_summary, filename="powerball_budget.xlsx"):
    ...
    # Explicitly utilize openpyxl format writing
    df_summary.to_excel(filename, index=False, engine='openpyxl')
...
with tab1:
    ...
    with col1:
        ...
        if st.button("🎲 Generate Random Quick Pick"):
            ...
            # FIXED: Saved index values individually inside cache storage mapping
            st.session_state["qp_m1"] = int(random_main[0])
            st.session_state["qp_m2"] = int(random_main[1])
            st.session_state["qp_m3"] = int(random_main[2])
            st.session_state["qp_m4"] = int(random_main[3])
            st.session_state["qp_m5"] = int(random_main[4])
            st.session_state["qp_pb"] = int(random_pb)
            
            # FIXED: Triggers a state refresh so your entry boxes show values instantly
            st.rerun()

        # Read back session memory configuration, default to system samples if empty
