...
st.set_page_config(page_title="National Lottery Ledger", page_icon="🗄️", layout="wide")
st.title("🗄️ Permanent National Lottery Ledger")
...
with col_m3:
    if total_records > 0:
        ...
        st.metric(label="Most-Played Game", value="—")

st.markdown("---")

tab1, tab2, tab3, tab5, tab4 = st.tabs(
    ["📝 Entry & Quick Pick", "📤 Smart Bulk Upload", "🔮 Ticket Checker Engine",
     "📅 Draw Results & Dividends", "✉️ Email & Settings"]
)
...
with tab1:
    ...
    with col1:
        ...
        st.caption(
            f"{game_choice}: pick {g['main_drawn']} numbers from 1–{g['main_pool']}"
            + (f", plus 1 {g['bonus_label']} from 1–{g['bonus_pool']}" if g["bonus_pool"] else "")
            + f"  •  Cost: R{selected_cost:.2f}"
        ...
        with qp_col1:
            single_clicked = st.button("🎲 Generate & Save Quick Pick")
        with qp_col2:
            all_clicked = st.button("🎲🎲 Generate One of Every Game")
        ...
        if st.session_state.get("qp_last_saved"):
            ...
            st.caption(
                f"🎯 This board's jackpot odds ({odds_game}): **1 in {jackpot_odds:,.0f}**  |  "
                f"Odds of winning *something*: **1 in {overall_odds:.1f}**"
            )
            with st.expander(f"📊 Full odds breakdown — {odds_game}"):
                st.dataframe(
                ...
                st.caption(
                    "These odds are identical for every possible board — the numbers you "
        ...
        manual_numbers_raw = st.text_input(
            f"Main numbers (comma-separated, {g['main_drawn']} numbers from 1–{g['main_pool']})",
            value="",
                        ...
                        results.append(
                            {
                                ...
                                "Bonus Match": "✅" if matched_bonus else "—",
                                "Result": outcome,
                            }
                        )
...
# ---------------------------------------------------------
with tab5:
    st.write("#### 📅 Log an Official Draw Result")
    st.caption(
        "There's no way for this app to fetch live results automatically — "
    ...
    with dr_col2:
        dr_numbers_raw = st.text_input(
            f"Winning numbers ({dg['main_drawn']} numbers, comma-separated, 1–{dg['main_pool']})",
    ...
    if dg["bonus_pool"]:
        dr_bonus_raw = st.number_input(
            f"Winning {dg['bonus_label']} (1–{dg['bonus_pool']})", 1, dg["bonus_pool"], 1, key="draw_bonus_input"
        )
    ...
    if st.button("💾 Save Draw Result & Auto-Check My Tickets"):
        try:
            ...
            if results is not None and not results.empty:
                ...
                if not won.empty:
                    st.success(f"🎉 {len(won)} ticket(s) won a total of R{won['Payout (ZAR)'].sum():,.2f}!")
            else:
    ...
    if history_df.empty:
        ...
        with st.expander("🗑️ Delete a draw result"):
            st.caption("Deleting a draw does not undo ticket winnings already applied — re-check manually if needed.")
